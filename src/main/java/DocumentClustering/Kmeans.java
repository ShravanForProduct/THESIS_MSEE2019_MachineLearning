...
//    }
    ...
    public static void startKMeans(List<Article> articles, int K, int maxIter, int numDocs) {
        Map<String, Map> docHashMap = HashMapBuilder.hashMapBuilder(articles, true);
        List<Document> docList = VectorRepresentation.convertToList(docHashMap);
        ...
        Map<Integer, List<Document>> clusterMap = Cluster.getClusters(centroids, docList);
        runKmeans(maxIter, centroids, docList, clusterMap);
        Helper.getDocuments(clusterMap, numDocs);
