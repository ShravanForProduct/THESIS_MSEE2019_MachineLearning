    ...
    private void print_TFIDF() {
        ...
        for (int i = 0; i < pool_of_Documents.size(); i++) {
            ...
            if (i == pool_of_Documents.size() - 1) {
                p = p + pool_of_Documents.get(i).getTitle() + " }";
            } else {
                p = p + pool_of_Documents.get(i).getTitle() + " ,";
            }
...
//                termsScore=termsScore.concat("    "+pool_of_Documents.get(c).countOccurences(total_words.get(i))+"  ");
    ...
    private void start_K_means(ArrayList<Article> pool_of_Documents, int cardinality2) throws Exception {
      ...
      { 
          ...
          {
              System.out.println(centroids.get(clusters).getdocs().get(doc).getTitle());
          }
          System.out.println("==========================================================");
          }
       }
    ...
    //KmeansCluster++
    private ArrayList<Centroid> calculate_random_initial_centroids(ArrayList<Article> pool_of_Documents,int cardinality1) {
        //Implementing KmeansCluster++ for initial seeding
